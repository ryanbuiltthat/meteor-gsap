 ...
 */
		...
		//now try to determine the requestAnimationFrame and cancelAnimationFrame functions and if none are found, we'll use a Meteor.setTimeout()/clearTimeout() polyfill.
		a = ["ms","moz","webkit","o"];
		i = a.length;
		...
		_class("Ticker", function(fps, useRAF) {
			var _self = this,
				...
				_tick = function(manual) {
					...
					if (manual !== true) { //make sure the request is made before we dispatch the "tick" event so that timing is maintained. Otherwise, if processing the "tick" requires a bunch of time (like 15ms) and we're using a Meteor.setTimeout() that's based on 16.7ms, it'd technically take 31.7ms between frames otherwise.
						_id = _req(_tick);
					}
					if (dispatch) {
			...
			//a bug in iOS 6 Safari occasionally prevents the requestAnimationFrame from working initially, so we use a 1.5-second timeout that automatically falls back to Meteor.setTimeout() if it senses this condition.
 ...
 */
		...
		//some browsers (like iOS) occasionally drop the requestAnimationFrame event when the user switches to a different tab and then comes back again, so we use a 2-second Meteor.setTimeout() to sense if/when that condition occurs and then wake() the ticker.
